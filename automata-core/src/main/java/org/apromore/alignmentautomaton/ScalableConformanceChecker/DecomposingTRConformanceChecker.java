...
public class DecomposingTRConformanceChecker {
  ...
  public DecomposingTRImporter decompositions;

  public UnifiedMap<Integer, TRConformanceChecker> componentAlignments = new UnifiedMap<Integer, TRConformanceChecker>();
  ...
  public void checkConformanceForComponents() {
    double start = System.currentTimeMillis();
    decompositions.minModelMoves = new TRConformanceChecker(decompositions.modelFSM).minModelMoves;
    int component;
    ...
    compList.parallelStream().forEach(sComp -> componentAlignments.put(sComp,
        new TRConformanceChecker(decompositions.componentDAFSAs.get(sComp), decompositions.sComponentFSMs.get(sComp),
            Integer.MAX_VALUE)));
		/*for(int pos=0; pos < decompositions.sComponentFSMs.size(); pos++)
		{
    ...
    //this.minModelMoves = ((new SingleTraceConformanceChecker(new IntArrayList(),decompositions.modelFSM,decompositions.globalLabelMapping, decompositions.globalInverseLabels, new UnifiedMap<>())).res.getInfo().get(PNRepResult.RAWFITNESSCOST));

  }

  public void recomposeConformance() {
    ...
    for (IntArrayList trace : decompositions.traceProjections.keySet()) {
      ...
      for (int pos = 0; pos <= trace.size(); pos++) {
        ...
        do {
          ...
          for (int label : relevantModelLabels.toArray()) {
            ...
            for (int comp : projectedAlignments.keySet()) {
              if (containsLabel(comp, label, ids.get(comp))) {
                if (projectedAlignments.get(comp).getStepTypesLst().get(0).get(ids.get(comp)) != StepTypes.MREAL ||
                    decompositions.globalInverseLabels
                        .get(projectedAlignments.get(comp).getNodeInstanceLst().get(0).get(ids.get(comp))) != label) {
                  isCoherent = false;
                } else {
                  actModelMoveComps.add(comp);
                }
              }
            }
            if (isCoherent) {
              ...
              for (int comp : actModelMoveComps.toArray()) {
                ids.put(comp, ids.get(comp) + 1);
              }
              stepAdded = true;
              modelMoves++;
              break;
          ...
          if (!stepAdded) {
            ...
            for (int ev : trace.toArray()) {
              xAttrLiteral = xFac
                  .createAttributeLiteral("concept:name", decompositions.globalInverseLabels.inverse().get(ev), xtend);
        ...
        for (it1 = 0; it1 < relevantComponents.size(); it1++) {
          ...
          for (int it2 = 0; it2 < relevantComponents.size(); it2++) {
            ...
            if (curEvent2 != event || curStep != curStep2) {
              ...
              for (int it3 = ids.get(comp2) + 1;
                  it3 < projectedAlignments.get(comp2).getNodeInstanceLst().get(0).size(); it3++) {
                ...
                if (curStep3 == StepTypes.MREAL || (curStep3 == StepTypes.LMGOOD && curEvent3 != event)) {
                  break;
                } else if (curStep3 == StepTypes.L && curEvent3 != event) {
                  continue;
                }
          ...
          if (isCoherent) {
            ...
            if (curStep == StepTypes.L) {
              logMoves++;
            }
            for (int compo : relevantComponents.toArray()) {
              ids.put(compo, ids.get(compo) + 1);
            }
            break;
          }
        }
        if (it1 == relevantComponents.size()) {
          conflictOccurred = true;
        }
        if (conflictOccurred) {
      ...
      for (int pos = 0; pos < res.getNodeInstanceLst().get(0).size(); pos++) {
        ...
        if (res.getStepTypesLst().get(0).get(pos) == StepTypes.L) {
          continue;
        }
        ...
        for (int curNode : toBeVisited.toArray()) {
          for (Transition tr : decompositions.modelFSM.states().get(curNode).outgoingTransitions()) {
            if (tr.eventID() == curEventID) {
              next.add(tr.target().id());
            }
          }
        }
        toBeVisited = next;
      ...
      res.getTraceIndex().addAll(Arrays.asList(relevantTraces));
      ...
      res.addInfo(PNMatchInstancesRepResult.TIME, System.currentTimeMillis() - start);
      res.addInfo(PNMatchInstancesRepResult.TRACEFITNESS,
          (double) 1 - res.getInfo().get(PNMatchInstancesRepResult.RAWFITNESSCOST) / (
              res.getInfo().get(PNMatchInstancesRepResult.ORIGTRACELENGTH) + this.minModelMoves));
    ...
    if (conflict > 0) {
      ...
      PNMatchInstancesRepResult rescueRes = new TRConformanceChecker(
          new ImportEventLog().createReducedDAFSAfromLog(rescueLog, decompositions.globalInverseLabels),
          decompositions.modelFSM, Integer.MAX_VALUE).resOneOptimal();
      ...
      for (AllSyncReplayResult rescueres : rescueRes) {
        listOfAlignmentsLabels = new ArrayList<List<Object>>();
        listOfAlignmentsOperations = new ArrayList<List<StepTypes>>();
        AllSyncReplayResult res2 = new AllSyncReplayResult(listOfAlignmentsLabels, listOfAlignmentsOperations, -1,
            true);
        ...
        Integer[] relevantTraces = ArrayUtils.toObject(
            decompositions.caseTracesMapping.get(rescueMapping.get(rescueres.getTraceIndex().first())).toArray());
        res2.getTraceIndex().addAll(Arrays.asList(relevantTraces));
  ...
  private boolean containsLabel(int comp, int label, int id) {
    ...
    if (alignment.getNodeInstanceLst().isEmpty()) {
      return false;
    }
  ...
  public void printAlignmentResults(String alignmentStatisticsFile, String caseTypeAlignmentResultsFile)
    ...
    pw1.println(
        this.alignmentResult.getInfo().get(PNMatchInstancesRepResult.RAWFITNESSCOST) + "," + this.alignmentResult
            .getInfo().get(PNMatchInstancesRepResult.ORIGTRACELENGTH) + "," + this.alignmentResult.getInfo()
            .get(PNMatchInstancesRepResult.QUEUEDSTATE) + "," + this.alignmentResult.getInfo()
            .get(PNMatchInstancesRepResult.NUMSTATES) + "," + this.alignmentResult.getInfo()
            .get(PNMatchInstancesRepResult.TIME) + "," + this.alignmentResult.getInfo()
            .get(PNMatchInstancesRepResult.NUMALIGNMENTS) + "," + this.alignmentResult.getInfo()
    ...
    for (IntArrayList traces : caseReplayResultMapping.keySet()) {
      ...
      for (int trace : traces.toArray()) {
        pw1.println(
            num++ + "," + caseType + "," + decompositions.caseIDs.get(trace) + "," + trace + "," + res.isReliable()
